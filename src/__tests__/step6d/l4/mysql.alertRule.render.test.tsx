/**
 * 第六步 第 4 段 · 阶段 1（W1-SQL组）· mysql 告警规则编辑器的渲染测试。
 *
 * 要证的事和阶段 0 的 ck 那条一样：从 fe v9.1.0 搬来的 mysql 告警编辑器，
...
jest.mock('@/plugins/mysql/services', () => ({
  __esModule: true,
...
// 智能告警的算法列表（只在 cate==='prometheus' 且开了 fcBrain 时才渲染，mysql 走不到，兜底 mock）
...
// 一条 mysql 策略的初值，再补上一条真实 SQL，用来验回填。
const mysqlStrategy = {
  cate: 'mysql',
  prod: 'metric',
  datasource_ids: [1],
...
function renderInStrategies(strategy: any = mysqlStrategy) {
  return render(
    <Form initialValues={{ strategies: [strategy] }}>
      <Form.List name='strategies'>
        {(fields) => (
          <>
            {fields.map((field) => (
              // 传法与真实分发点一致：src/pages/alertRules/Form/Rule/Rule/Metric/index.tsx 的 cate === 'mysql' 分支
              <MysqlAlertRule key={field.key} field={field} cate='mysql' datasourceValue={[1]} />
            ))}
          </>
        )}
      </Form.List>
...
describe('阶段 1 · mysql 告警规则编辑器接进羚牛多策略表单', () => {
  it('渲染不抛异常', () => {
  ...
  it('查询卡片的字段落在 strategies[0].rule_config.queries 下（不是表单顶层）', () => {
    ...
    // 查询卡片里的「高级设置」两个字段（src/plugins/mysql/components/AdvancedSettings.tsx:61,94），
  ...
  it('exp_trigger_disable 为 true 时，触发条件那块收起来（反证 useWatch 真的读到了值）', () => {
    const { container } = renderInStrategies({
      ...mysqlStrategy,
      rule_config: { ...mysqlStrategy.rule_config, exp_trigger_disable: true },
    });
    expect(container.querySelector('#strategies_0_rule_config_exp_trigger_disable')).not.toBeNull();
