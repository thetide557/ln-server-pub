/**
 * 第六步 第 4 段 · 阶段 1（W1-时序组）· IoTDB 告警规则编辑器的渲染测试。
 *
 * 要证的事和阶段 0 的 ck 那条一模一样：
 * 从 fe v9.1.0 搬来的 IoTDB 告警编辑器，放进羚牛的「一条规则最多 5 个策略」表单里
...
// GraphPreview（查询结果预览）和 MetaModal（库表树）一挂载就发请求；这里换成空数据，
// 免得测试里跑真网络。函数名照 src/plugins/iotdb/services.ts 的 export 列表。
...
// 智能告警的算法列表（只在 cate==='prometheus' 且开了 fcBrain 时才渲染，iotdb 走不到，兜底 mock）
...
// 一条 iotdb 策略的初值。rule_config 的形状照 fe v9.1.0 的 iotdb 编辑器
...
const iotdbStrategy = {
  ...
  rule_config: {
    queries: [
      {
        ref: 'A',
        query: 'select time, v from root.t',
        interval: 1,
        interval_unit: 'min',
        keys: {
          metricKey: ['v'],
...
function renderInStrategies(strategy: any = iotdbStrategy) {
  return render(
        ...
        {(fields) => (
          <>
            {fields.map((field) => (
              ...
              // src/pages/alertRules/Form/Rule/Rule/Metric/index.tsx 的 cate === 'iotdb' 分支
              <IotDBAlertRule key={field.key} field={field} cate='iotdb' datasourceValue={[1]} />
            ))}
          </>
        )}
      </Form.List>
...
describe('阶段 1 · iotdb 告警规则编辑器接进羚牛多策略表单', () => {
  it('渲染不抛异常', () => {
  ...
  it('查询卡片的字段落在 strategies[0].rule_config.queries 下（不是表单顶层）', () => {
    ...
    // 查询卡片里「高级设置」的三个字段（src/plugins/iotdb/components/AdvancedSettings.tsx:56,72,90；
    // timeKey 那一块由 datasourceCate === 'iotdb' 打开，Queries/index.tsx 正是这么传的），
    ...
    expect(container.querySelector('#strategies_0_rule_config_queries_0_keys_timeKey')).not.toBeNull();
  ...
  it('已有的 SQL 能回填进编辑器', () => {
    const { container } = renderInStrategies();
    expect(container.innerHTML).toContain('select time, v from root.t');
  });
  ...
  it('exp_trigger_disable 为 true 时，触发条件那块收起来（反证 useWatch 真的读到了值）', () => {
    const { container } = renderInStrategies({
      ...iotdbStrategy,
      rule_config: { ...iotdbStrategy.rule_config, exp_trigger_disable: true },
    });
    // 总开关还在（它在卡片头上，不受折叠影响）
    expect(container.querySelector('#strategies_0_rule_config_exp_trigger_disable')).not.toBeNull();
    // 但里面的触发条件卡片不该渲染
