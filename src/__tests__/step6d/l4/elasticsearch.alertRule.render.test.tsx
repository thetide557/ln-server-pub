/**
 * 第六步 第 4 段 · 阶段 1（W1-ES 组）· elasticsearch 告警规则编辑器的渲染测试。
 *
...
function renderInStrategies(strategy: any = esStrategy) {
  return render(
    <Form initialValues={{ strategies: [strategy] }}>
      <Form.List name='strategies'>
        {(fields) => (
          <>
            {fields.map((field) => (
              // 传法与真实分发点一致：cate === 'elasticsearch' 时不传 hideIndexPattern
              // （fe v9.1.0 src/pages/alertRules/Form/Rule/Rule/index.tsx:44 就是这么分的）
              <ElasticsearchAlertRule key={field.key} field={field} cate='elasticsearch' datasourceValue={[1]} disabled={false} />
            ))}
          </>
        )}
      </Form.List>
...
describe('阶段 1 · elasticsearch 告警规则编辑器接进羚牛多策略表单', () => {
  it('渲染不抛异常', () => {
  ...
  it('elasticsearch 会去拉索引模式列表（opensearch 那条会反过来断言不拉）', () => {
    renderInStrategies();
    // Query.tsx:56-61：只有 hideIndexPattern 为假时才请求索引模式列表。
    // elasticsearch 不传 hideIndexPattern，所以这里要拉；opensearch 传了，那条测试断言一次都不拉。
    // 这一条同时说明「索引模式」这条路在 elasticsearch 下是通的。
    expect(getESIndexPatterns).toHaveBeenCalledWith(1);
  });
  ...
  it('exp_trigger_disable 为 true 时，触发条件那块收起来（反证 useWatch 真的读到了值）', () => {
    const { container } = renderInStrategies({
      ...esStrategy,
      rule_config: { ...esStrategy.rule_config, exp_trigger_disable: true },
    });
    expect(container.querySelector('#strategies_0_rule_config_exp_trigger_disable')).not.toBeNull();
