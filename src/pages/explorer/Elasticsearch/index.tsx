...
// step6f（ES 升级轮）：fe v9.1.0 的时序图组件收一个 `hideResetBtn`（把「重置缩放」按钮藏起来），
// 羚牛 pub 那份没有这个 prop。pub 现有文件不许改，这里把类型放宽成 any——
// 运行时 pub 的组件会忽略它，也就是图上那个重置按钮照常显示。已登记。
import _Timeseries from '@/pages/dashboard/Renderer/Renderer/Timeseries';
const Timeseries: any = _Timeseries;
import { CommonStateContext } from '@/App';
// step6f（ES 升级轮）：fe v9.1.0 的 @/utils/constant 里有 PRIMARY_COLOR（主题主色 '#6C53B1'，fe constant.ts:86），
// 羚牛 pub 的同名文件里没有；pub 现有文件不许改，这里就地放一个同值的常量。已登记。
const PRIMARY_COLOR = '#6C53B1';
...
const HeaderExtra = ({ mode, setMode, allowHideSystemIndices, setAllowHideSystemIndices, datasourceValue, form }) => {
  ...
  // step6f（ES 升级轮）：fe 的 ICommonState 有 esIndexMode（全局设定 ES 查询用「索引」还是「索引模式」），
  // pub 的没有（`src/App.tsx:73-108`）。pub 现有文件不许改，加 as any：pub 上取到 undefined，
  // 下面的 `if (esIndexMode === 'index-patterns' || esIndexMode === 'indices')` 就不成立，
  // 两种模式都留给用户自己在页面上切——正是 pub 现在的表现。已登记。
  const { esIndexMode, isPlus } = useContext(CommonStateContext) as any;
