...
// step6f（ES 升级轮）：fe v9.1.0 这里是 `import { basePrefix } from '@/App'`
//（fe App.tsx:135 `export const basePrefix = import.meta.env.VITE_PREFIX || ''`，路由的统一前缀）。
// 羚牛 pub 的 @/App 没有这个导出，而且本文件从头到尾一次都没用它，所以直接去掉这行 import。已登记。
...
// ================================================================================
// step6f（ES 升级轮）：下面两个函数是**羚牛这边补回来的**，不是 fe v9.1.0 的内容。
// 原因：pub 有两个四层之外的文件还在从这里取它们，而 fe v9.1.0 的 utils 里已经没有了——
//   - `src/pages/explorer/components/FieldsSidebar/Field.tsx:7` 取 normalizeFieldValuesQueryRequestBody
//   - `src/pages/log/IndexPatterns/Fields.tsx:27` 取 getFieldType
// 按本轮纪律那两个文件不许改，所以把这两个函数原样搬回来（取自覆盖前的
// `src/pages/explorer/Elasticsearch/utils.tsx`，即本仓库提交 eb6c9d0 里的那一版，逐字未改）。
// 将来若把 FieldsSidebar / IndexPatterns 也升到 v9，这一段就可以删掉。
// ================================================================================
