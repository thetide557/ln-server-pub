...
import { getInspectionList, addInspection, editInspection, removeInspection, getInspectionLogList, getInspectionReportList } from '@/services/sxxc/inspection'
...
const Resource: React.FC = () => {
    ...
    const changeReport = (val) => {
        console.log(val)
        history.push(`/inspection/inspectionReport/${val.id}`);
        // let params = { id: val.id }
        // getInspectionReportList(params).then(res => {
        //     console.log(res)
        //     setReport(res.data)
        //     setVisible(true)
        // })
    ...
    const getTableData = ({ current, pageSize }): Promise<any> => {
        const params = {
            ...
            scopeContext: checkedServe.length != 0 ? checkedServe.join(',') : undefined,
            // checkedIp: checkedIp.length != 0 ? checkedIp.join(',') : undefined,
            hosts: checkedIp.length != 0 ? checkedIp.join(',') : undefined,
            pageSize: pageSize,
            pageNum: current,
