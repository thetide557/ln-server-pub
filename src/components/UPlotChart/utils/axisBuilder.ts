...
function formatTime(self: uPlot, splits: number[], axisIdx: number, foundSpace: number, foundIncr: number): string[] {
  ...
  // step6d(B1)：pub 装的 uplot 1.6.32 的 Axis 类型里没有 timeZone 字段（fe 那边 TS 是 4.9、装的也是 1.6.x，
  // 但这行本来就是运行时用 'in' 判断有没有这个字段的写法），TS 4.3 只认类型不认 in 收窄，所以加 as any。
  const timeZone = 'timeZone' in axis && typeof (axis as any).timeZone === 'string' ? ((axis as any).timeZone as string) : undefined;
