...
export default function Query(props: Props) {
  ...
  const { run: onIndexChange } = useDebounceFn(
    (val) => {
      if (datasourceValue && val) {
        getFullFields(datasourceValue, val,  'date').then((res) => {
  ...
  return (
    <div
      key={field.key}
      style={{ position: "relative", backgroundColor: '#fafafa', padding: 16,marginBottom:16 }}
    >
      <Row gutter={8}>
        <Col flex="32px">
                  ...
                  <span>
                    ...
                    <Tooltip
                      title={
                        <Trans
                          ns="datasource"
                          i18nKey="datasource:es.index_tip"
                          components={{ 1: <br /> }}
                        />
                      }
                    >
              ...
              <InputGroupWithFormItem
                label={
                  <span>
                    ...
                    <FilterHelp visible={filterHelpVisible} onClose={() => setFilterHelpVisible(false)} />
                  </span>
                }
                labelWidth={90}
              >
