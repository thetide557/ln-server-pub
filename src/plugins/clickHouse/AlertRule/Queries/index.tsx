...
// 第六步 第4段 W0：pub 锁的 lucide-react 是 0.294.0，里面还没有 WandSparkles 这个名字
// （node_modules/lucide-react/dist/lucide-react.d.ts 里只有 Wand / Wand2），换成同类的 Wand2。
// 升 lucide-react 属依赖升级，本轮不做（登记在 拿不准-W0.md U-04）。
...
interface IProps {
  form: any;
  prefixField?: any;
  fullPrefixName?: (string | number)[]; // 完整的前置字段名，用于 getFieldValue 获取指定字段的值。第六步 第4段 W0：类型从 string[] 放宽到 (string|number)[]——羚牛的前缀里带策略下标（数字）
  prefixName?: (string | number)[]; // 列表字段名（相对路径）
  disabled?: boolean;
  datasourceValue: number | number[];
  // 第六步 第4段 W0：fe 原来在下面那处 shouldUpdate 里，直接读**顶层**的 cate 字段；
...
export default function index({ form, prefixField = {}, fullPrefixName = [], prefixName = [], disabled, datasourceValue, cate }: IProps) {
  ...
  // 第六步 第4段 W0（规矩 A）：useWatch 走绝对路径。fe 原文写死 ['rule_config','queries']；
              ...
              return (
                <CardContainer key={field.key} onClose={fields.length > 1 ? () => remove(field.name) : undefined}>
                  ...
                  <Form.Item shouldUpdate noStyle>
                    {({ getFieldValue }) => {
                      // 第六步 第4段 W0：fe 原文这里是从表单**顶层**取 cate 字段，
