                      ...
                      <Col flex='none'>
                        <SqlTemplates
                          onSelect={(sql) => {
                            ...
                            // fe:src/plugins/TDengine/AlertRule/Queries/index.tsx:132-141——先 cloneDeep 整个 queries 数组、
                            // _.set 改一项，再用 setFieldsValue 把整个 rule_config 覆盖回去。两个毛病：
                            // ① getFieldValue / setFieldsValue 是绝对路径调用，却写死表单顶层那一层，
                            //    羚牛这里应该是 ['strategies', n, 'rule_config']；
                            // ② 整块覆盖会把同一策略里别人（触发条件那一块）刚改的值一起写回旧值。
                            // 改成 setFields 只写要改的那一个键。
                            form.setFields([{ name: [...fullPrefixName, ...prefixName, 'queries', field.name, 'query'], value: sql }]);
                          }}
                        />
                      </Col>
                      <Col flex='none'>
                        <MetaModal
                          ...
                          onTreeNodeClick={(nodeData) => {
                            // 第六步 第4段 W1（规矩 A + 大顾问 Q1 第三节点名）：同上，fe 原文整块覆盖 rule_config，
                            // 改成 setFields 逐键写。原文里 keys 是「取旧的 keys 再展开、只换 metricKey」，
                            // 直接写到 ['keys','metricKey'] 这一层等价，且不用先读旧值。
                            const queryPath = [...fullPrefixName, ...prefixName, 'queries', field.name];
                            const nextFields: { name: (string | number)[]; value: any }[] = [
                              { name: [...queryPath, 'query'], value: `select * from ${nodeData.database}.${nodeData.table} where _ts >= $from and _ts < $to` },
                            ];
